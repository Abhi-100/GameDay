...
SUPPORTED_DOCUMENT_TYPES = {'.pdf', '.txt', '.doc', '.docx', '.md', '.rtf'}


class Attachment:
    """Simple attachment class for handling files"""
    ...
    def is_image(self) -> bool:
        ...
        ext = f".{self.file_type.lower()}"
        return ext in SUPPORTED_IMAGE_TYPES
    
    def sanitize_document_name(self, filename: str) -> str:
        """Remove extension from filename for API payload"""
        if '.' in filename:
            return '.'.join(filename.split('.')[:-1])
        return filename


def create_payload_with_attachments(prompt: str, attachments: List[Attachment]) -> Dict:
    """Create API payload with attachments following the expected format"""
    if not prompt or not prompt.strip():
        raise ValueError("Prompt is required - cannot send files without accompanying text")
    
    # Start with mandatory prompt
    ...
    if attachments:
        ...
        documents = [att for att in attachments if not att.is_image()]
        
        if images:
            payload["images"] = [{
                "name": att.sanitize_document_name(att.filename),
                "format": att.file_type.lower(),
...
def invoke_agent_streaming(
    ...
    try:
        ...
        # Create payload with attachments if provided
        if attachments:
            payload_data = create_payload_with_attachments(prompt, attachments)
        else:
        ...
        boto3_response = agentcore_client.invoke_agent_runtime(
            ...
            runtimeUserId=sts_client.get_caller_identity()["UserId"],
...
def main():
    ...
    # Initialize file uploader counter to clear attachments after sending
    ...
    # File uploader for attachments (placed just above chat input)
    # Using a dynamic key to clear the uploader after each message
    uploaded_files = st.file_uploader(
        "📎 Attach files (images or documents)",
        accept_multiple_files=True,
        type=['jpg', 'jpeg', 'png', 'gif', 'bmp', 'webp', 'pdf', 'txt', 'doc', 'docx', 'md', 'rtf'],
        help="Upload images (jpg, png, gif, etc.) or documents (pdf, txt, doc, etc.)",
    ...
    if uploaded_files:
        ...
        with cols[1]:
            ...
            for uploaded_file in uploaded_files:
                ...
                if file_ext in SUPPORTED_IMAGE_TYPES or file_ext in SUPPORTED_DOCUMENT_TYPES:
                    # Create attachment object
                    attachment = Attachment(uploaded_file.name, uploaded_file.read())
                    attachments.append(attachment)
                    
                    # Collect file info for compact display
                    file_type = "🖼️" if attachment.is_image() else "📄"
                    attachment_names.append(f"{file_type} {uploaded_file.name}")
                else:
    ...
    if prompt := st.chat_input("Type your message here..."):
        ...
        # Validate that we have a prompt when sending attachments
        if attachments and (not prompt or not prompt.strip()):
            st.error("Please enter a message when sending attachments.")
            return
        ...
        if attachments:
            ...
            for att in attachments:
                file_type = "🖼️" if att.is_image() else "📄"
                attachment_info.append(f"{file_type} {att.filename}")
            display_message += f"\n\n**Attachments:** {', '.join(attachment_info)}"

        # Add user message to chat history
        st.session_state.messages.append(
            {"role": "user", "content": display_message, "avatar": HUMAN_AVATAR}
        )
        ...
        # Clear file uploader by incrementing the key counter
