...
def is_s3_uri(path: str) -> bool:
    """Determine if a given path is an S3 URI.

    Args:
        path: The path to check.

    Returns:
        True if the path is an S3 URI (starts with "s3://"), False otherwise.
    """
    return path.startswith('s3://')


def validate_s3_object_exists(s3_uri: str) -> None:
    """Validate that an S3 object exists at the given URI.

    Args:
        s3_uri: The S3 URI to validate.

    Raises:
        ValueError: If the URI is malformed, the object does not exist, or AWS API call fails.
    """
    try:
        # Parse the S3 URI
        bucket, key = get_bucket_and_key_from_s3_uri(s3_uri)
        
        # Validate URI format has both bucket and key components
        if not bucket or not key:
            logger.error(f'Invalid S3 URI format: {s3_uri}. Missing bucket or key.')
            raise ValueError(f'Invalid S3 URI format: {s3_uri}. Expected format: s3://bucket-name/key')
        
        # Use S3 client's head_object to verify object exists
        s3_client = get_s3_client()
        s3_client.head_object(Bucket=bucket, Key=key)
        
        logger.info(f'S3 object validated: {s3_uri}')
        
    except ValueError:
        # Re-raise ValueError from format validation
        raise
    except Exception as e:
        # Handle AWS ClientError and other exceptions
        error_message = str(e)
        
        # Check if it's a 404 error (object not found)
        if hasattr(e, 'response') and e.response.get('Error', {}).get('Code') == '404':
            logger.error(f'S3 object not found at {s3_uri}')
            raise ValueError(f'S3 object not found at {s3_uri}')
        
        # Handle other AWS errors
        logger.error(f'Error validating S3 object at {s3_uri}: {error_message}')
        raise ValueError(f'Error validating S3 object: {error_message}')
...
async def invoke_data_automation_and_get_results(
    ...
    Args:
        asset_path: The path to the asset to process (local file path or S3 URI).
    ...
    """
    # Determine if S3 URI or local path
    if is_s3_uri(asset_path):
        logger.info(f'S3 URI provided: {asset_path}')
        validate_s3_object_exists(asset_path)
        asset_uri = asset_path
        logger.info('Skipping upload, using provided S3 URI')
    else:
        logger.info(f'Local file path provided: {asset_path}')
